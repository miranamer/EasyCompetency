...
const Experiences = ({ experiences, setCurrentPage, setExperiences }) => {
  ...
  const removeMe = (index) => {
    ...
    chrome.storage.local.set({ key: experiences.filter((_, i) => i !== index) }).then(() => {
  ...
  return (
    <>
      <div className="flex h-full w-full p-10 overflow-auto text-white font-semibold relative">
        {experiences.length <= 0 ? <h1 className="text-2xl font-bold text-gray-400 relative bottom-3">Add An Experience</h1> : <div className="flex flex-col gap-10 m-10">
